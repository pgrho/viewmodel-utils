...
foreach($gsd in @("./Styles/", "./Scripts/", $toastAssets, $typeaheadAssets)) {
    $dp = $gsd
    if ($dp.StartsWith(".")) {
        $dp = "$projectDir$gsd"
    }
...
foreach($df in @("wwwroot/Shipwreck.ViewModelUtils.Blazor.bundle.css")) {
...
if ($srcTime -lt $destTime) {
    ...
    cd $projectDir
    cd node_modules\.bin\
    .\gulp.CMD --toast "$toastAssets" --typeahead "$typeaheadAssets"
}